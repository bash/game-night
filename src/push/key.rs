...
use std::io;
use std::path::Path;
use web_push::jwt_simple::prelude::P256KeyPair;

pub(crate) struct WebPushKey(P256KeyPair);
...
impl WebPushKey {
    ...
    pub(crate) fn public_key(&self) -> Vec<u8> {
        self.0.public_key().to_bytes_uncompressed()
    }
}
...
impl crate::fs::GeneratedFile for WebPushKeyFile {
    ...
    fn generate(&mut self) -> Self::Value {
        WebPushKey(P256KeyPair::generate())
    }
    ...
    fn read(&self, read: &mut dyn io::Read) -> Result<Self::Value> {
        ...
        Ok(WebPushKey(P256KeyPair::from_pem(&pem)?))
    }
}
